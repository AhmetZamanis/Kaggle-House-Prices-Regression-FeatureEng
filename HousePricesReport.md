...
The package uses mutual information as the metric of both relevancy and
redundancy: Relevancy is the mutual information of the predictor with
the target variable, while redundancy is the average mutual information
of the predictor with the other previously evaluated predictors. The
MRMR score for a predictor is then calculated by subtracting redundancy
from relevancy. Therefore, negative MRMR scores are possible, but do not
necessarily mean the predictor has no information on the target
variable.  
...
mim_tab$Redundancy <- mim_tab$Relevancy - mim_tab$MRMRScore
...
-   These are likely because the calculation of redundancy, and
    ...
    -   The calculation for the second predictor takes into account only
        ...
        third predictor takes into account theaverage redundancy with
...
-   Keep in mind that mutual information does not have a hard limit,
    unlike a maximum correlation of 1. Zero mutual information means
    just that, while a larger value means more information in common
...
-   xgb.cv is primarily useful for fine tuning nrounds: The number of
    ...
    -   Generally, too many boosting rounds will lead to overfitting:
        ...
        increase or remain stuck.
    -   To prevent this, we use the early_stopping_rounds parameter:
        Boosting stops after an N number of rounds with no improvement
